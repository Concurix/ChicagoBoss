...
route(Config, "") ->
    route(Config, "/");
route(#boss_router_config{application = AppName, routes_table_id = RoutesTable}=Config, Url) ->
    Route = case get_match(Url, ets:tab2list(RoutesTable)) of
        undefined -> 
...
unroute(#boss_router_config{application = AppName, routes_table_id = RoutesTable}, Controller, Action, Params) ->
...
handle(Config, StatusCode) ->
    case ets:lookup(Config#boss_router_config.handlers_table_id, StatusCode) of
        [] ->
            not_found;
...
load(#boss_router_config{application = AppName, routes_table_id = RoutesTable,
                         handlers_table_id = HandlersTable}) ->
    RoutesFile = boss_files:routes_file(AppName),
        ...
        {ok, OrderedRoutes} -> 
            ...
            lists:map(fun
                    ({Number, UrlOrStatusCode, Proplist}) when is_list(Proplist) ->
                        TheApplication = proplists:get_value(application, Proplist, AppName),
                        ...
                        case UrlOrStatusCode of
                            Url when is_list(Url) ->
                                ...
                                NewRoute = #boss_route{ 
                                    ...
                                    action = TheAction, 
                                    params = CleanParams },
                                true = ets:insert(RoutesTable, NewRoute);
                            StatusCode when is_integer(StatusCode) ->
                                NewHandler = #boss_handler{ 
                                    ...
                                    action = TheAction, 
                                    params = CleanParams },
                                true = ets:insert(HandlersTable, NewHandler)
                        end
                end, Routes);
        Error -> 
